...
class LapisSymbolProcessor(
    ...
    private val poetesse = Poetesse {
        ...
        javaNullabilityResolver = KspJavaNullabilityResolver(
            nullableAnnotationClassName = kspOptions.nullableAnnotation?.let { JPClassName.bestGuess(it) },
            nonNullAnnotationClassName = kspOptions.nonNullAnnotation?.let { JPClassName.bestGuess(it) },
        )
    }

    private val lowering: Lowering by lazy { Lowering(kspOptions, poetesse, logger) }
    ...
    private fun generate() {
        ...
        Generator(kspOptions, poetesse, codeGenerator, logger).generate(patches.values.toList())
    }
}
